                                 ...
                                 int tolerance, int frameFreq)
{
     ...
     */
    solidColorPicker Solidcolors = solidColorPicker(fillColor);

    return fill(img,x,y,Solidcolors,tolerance,frameFreq);
}
                                ...
                                int gridSpacing, int tolerance, int frameFreq)
{
     ...
     * correct call to fill with the correct colorPicker parameter.
     */
    gridColorPicker GridColor = gridColorPicker(gridColor,gridSpacing);
    return fill(img,x,y,GridColor,tolerance,frameFreq);
}
                                    ...
                                    int radius, int tolerance, int frameFreq)
{
    ...
    gradientColorPicker GradientColor = gradientColorPicker(fadeColor1,fadeColor2,radius,x,y);
    return fill(img,x,y,GradientColor,tolerance,frameFreq);
}
                       ...
                       int tolerance, int frameFreq)
{
    ...
    int w = img.width();
    int h = img.height();
    //vector<bool> processed(w*h);
    vector<vector<bool>> processed (w,vector<bool>(h,false));

    RGBAPixel temp = *img(x,y);
    int r = temp.red;
    int g = temp.green;
    int b = temp.blue;
    ...
    {
      int newx = xcoord.remove();
      int newy = ycoord.remove();
      RGBAPixel *current = img(newx,newy);
      ...
      int distance = pow((red - r),2) + pow((green - g),2) + pow((blue - b),2);

      if (distance <= tolerance && !processed[newx][newy])
      {
        processed[newx][newy] = true;
        count++;
        *img(newx,newy) = fillColor(newx,newy);
